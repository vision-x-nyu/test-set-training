...
def fuzzy_cleanup_NA(pred):
    if t2d is None:
        raise ImportError(
            "text2digits is not installed. Please install it to use this function."
        )
...
def evaluate_models(df, model_columns=None, sort=False):
    ...
    if sort:
        type_scores = type_scores[
            sorted(
                type_scores.columns,
                key=lambda col: type_scores.loc["overall", col],
                reverse=False,
            )
        ]
...
def visualize_model_scores(df, model_columns=None):
    ...
    sns.heatmap(
        ranks, annot=scores, fmt=".2f", cmap="coolwarm", cbar=True, linewidths=0.125
    )
    plt.xlabel("Model")
...
def visualize_debiasing_impact(original_df, debiased_df, model_columns=None, title=""):
    ...
    assert n_rows == len(debiased_scores), (
        f"Number of rows in original and debiased scores do not match: {n_rows} != {len(debiased_scores)}"
    )
...
def get_object_size_estimation_scores_v2(
    ...
    qdf["log_ground_truth"] = np.log(
        qdf["ground_truth_num"] + 1e-6
    ...
    obj_stats["log_ratio"] = (
        obj_stats["log_std"] / (obj_stats["log_mean"] + epsilon)
    ).fillna(0)
    ...
    df_merged["log_inv_var_score"] = 1.0 - minmax_scale(
        df_merged["log_ratio"] + epsilon
    )

    # Closeness to Mean Component (closer = higher score)
    # Normalize distance by std dev, then invert (1 - normalized_distance)
    df_merged["norm_dist_from_mean"] = abs(
        df_merged["ground_truth_num"] - df_merged["mean"]
    ) / (df_merged["std"] + epsilon)
    df_merged["log_norm_dist_from_mean"] = abs(
        df_merged["log_ground_truth"] - df_merged["log_mean"]
    ) / (df_merged["log_std"] + epsilon)

    # Scale normalized distance to 0-1, then invert
    df_merged["obj_mean_dist_score"] = 1.0 - minmax_scale(
        df_merged["norm_dist_from_mean"]
    )
    df_merged["log_obj_mean_dist_score"] = 1.0 - minmax_scale(
    ...
    df_merged["log_global_mean_dist"] = abs(
        df_merged["log_ground_truth"] - log_global_mean
    )
...
def filter_object_size_estimation_scored_v2(
    ...
    """
    df_merged = get_object_size_estimation_scores_v2(
        df,
        w_freq=w_freq,
        w_ratio=w_ratio,
        w_dist=w_dist,
        w_glob_dist=w_glob_dist,
        logscale=logscale,
    )
    ...
    assert len(remove_ids) <= budget, (
        f"Exceeded budget in filtering process: {len(remove_ids)} > {budget}"
    )
...
def get_object_counting_score_v3(
    ...
    combo_counts = (
        qdf.groupby(["object", "ground_truth_str"])
        .size()
    ...
    df_merged = pd.merge(
        df_merged, combo_counts, on=["object", "ground_truth_str"], how="left"
    )
...
def filter_object_counting_dynamic_pct(
    ...
    """
    df_with_scores = get_object_counting_score_v3(
        df, w_obj_freq=w_obj_freq, w_combo_freq=w_combo_freq
    )
    ...
    ids_by_gt_val_sorted = (
        df_sorted.groupby("ground_truth_str")["id"].apply(list).to_dict()
    )
    ...
    if verbose:
        print(f"Starting counts: {current_counts}")

    while len(remove_ids) < budget:
        if total_remaining <= 1:
            break  # Avoid division by zero if only one sample left

        # Calculate current percentages
        current_percentages = {
            val: count / total_remaining for val, count in current_counts.items()
        }

        # Find GT values exceeding the threshold
        eligible_vals = {
            val: pct
            for val, pct in current_percentages.items()
        ...
        if not eligible_vals:
            if verbose:
                print(
                    f"No GT value exceeds threshold {max_pct_threshold:.1%}. Stopping."
                )
        ...
        if id_to_remove is None:
            ...
            if verbose:
                print(
                    f"Warning: Could not find an available ID for GT value {val_to_reduce}. Stopping."
                )
        ...
        if verbose and (len(remove_ids) % 10 == 0 or len(remove_ids) == budget):
            ...
            formatted_percentages = {
                key: f"{value:.1%}" for key, value in current_percentages.items()
            }
            print(
                f"Removed {len(remove_ids)}/{budget}. \tCurrent pcts: {formatted_percentages}"
            )
    ...
    if verbose:
        ...
        print(
            f"- Final percentages: \t{ {k: f'{v:.1%}' for k, v in final_percentages.items()} }"
        )
...
def get_object_abs_distance_scored_log_global(
    ...
    epsilon = 1e-6
    valid_distances = df_filtered["ground_truth_num"][
        df_filtered["ground_truth_num"] > 0
    ]
    ...
    if len(valid_distances) != len(df_filtered["ground_truth_num"]):
        raise ValueError(
            "Some distances are zero or negative, which is not allowed. Please check your data."
        )

    # shift by 1 so that all distances are positive
    df_filtered["log_ground_truth_num"] = np.log10(
        df_filtered["ground_truth_num"] + 1.0
    )

    # Group by object pair and compute stats
    pair_stats = (
        df_filtered.groupby("object_pair")
        .agg(
            count=("id", "count"),
    ...
    pair_stats["ratio_log"] = (
        pair_stats["std_log"] / (pair_stats["mean_log"] + epsilon)
    ).fillna(0)
    ...
    df_merged["pair_inv_var_score"] = 1.0 - minmax_scale(
        df_merged["ratio_log"] + epsilon
    )

    # Closeness to Pair Mean Component (in log-space)
    df_merged["norm_dist_from_pair_mean_log"] = abs(
        df_merged["log_ground_truth_num"] - df_merged["mean_log"]
    ) / (df_merged["std_log"] + epsilon)
    df_merged["pair_mean_dist_score"] = 1.0 - minmax_scale(
        df_merged["norm_dist_from_pair_mean_log"] + epsilon
    )

    # Global Typicality Component (using distance from global mean in log-space)
    df_merged["global_dist_from_mean_log"] = abs(
        df_merged["log_ground_truth_num"] - global_mean_log
    ) / (global_std_log + epsilon)

    # Normalize the global distance from mean
    df_merged["global_mean_dist_score"] = 1.0 - minmax_scale(
        df_merged["global_dist_from_mean_log"] + epsilon
    )
...
def get_rel_direction_removal_candidates(
    df_subtype: pd.DataFrame, seed: int = 42, sampled: bool = False
) -> dict:
    """
    ...
    for gt_answer, count in gt_counts.items():
        ...
        if num_to_remove > 0:
            ...
            removal_candidates[gt_answer] = ids_for_gt[
                :num_to_remove
            ]  # Take the first N excess IDs
...
def filter_object_rel_direction_v2(
    ...
    for subtype in subtypes:
        ...
        df_subtype["gt_idx"] = df_subtype["ground_truth"].apply(
            lambda x: "ABCD".index(x)
        )
        df_subtype["gt_option"] = df_subtype.apply(
        ...
        subtype_candidates = get_rel_direction_removal_candidates(
            df_subtype, seed=seed, sampled=shuffled
        )
    ...
    if verbose:
        print(
            f"Total samples identified for full balancing: {total_candidates_for_balance}"
        )
    ...
    if verbose:
        print(
            f"Actual number of samples selected for removal to balance: {len(final_remove_ids)}"
        )
    ...
    if len(final_remove_ids) > budget:
        if verbose:
            print(
                f"Warning: Balancing requires {len(final_remove_ids)} removals, exceeding budget {budget}. Selecting subset."
            )
...
def get_object_rel_distance_score_v2(
    ...
    qdf[["object_1", "object_2", "object_3", "object_4", "target_object"]] = qdf[
        "question"
    ].str.extract(
    ...
    qdf["tgt_gt_pair"] = qdf.apply(
        lambda row: "-".join(sorted([row["target_object"], row["gt_object"]])), axis=1
    )
    qdf["tgt_gt_ord_pair"] = qdf.apply(
        lambda row: "-".join([row["target_object"], row["gt_object"]]), axis=1
    )
    ...
    for index, row in qdf.iterrows():
        ...
        for opt_obj in option_objects:
            ...
            tgt_option_ord_pair_freqs.append(
                global_ordered_pair_counts.get(tgt_option_ord_pair, 0)
            )
...
def filter_object_rel_distance_score_v2(
    ...
    df_filtered = get_object_rel_distance_score_v2(
        df, w_gt_obj, w_pair_freq, w_ord_pair_freq
    )
    if df_filtered.empty:
        return []
...
def get_app_order_relative_score_v2(
    ...
    scaled_pos_freq_values = (
        minmax_scale(pos_freq_values)
        if len(np.unique(pos_freq_values)) > 1
        else np.ones_like(pos_freq_values)
    )
    scaled_pair_freq_values = (
        minmax_scale(pair_freq_values)
        if len(np.unique(pair_freq_values)) > 1
        else np.ones_like(pair_freq_values)
    )
    scaled_comb_pair_freq_values = (
        minmax_scale(comb_pair_freq_values)
        if len(np.unique(comb_pair_freq_values)) > 1
        else np.ones_like(comb_pair_freq_values)
    )

    norm_pos_freq_map = {
        key: scaled_pos_freq_values[i][0] for i, key in enumerate(position_freq.keys())
    }
    norm_pair_freq_map = {
        key: scaled_pair_freq_values[i][0] for i, key in enumerate(pair_freq.keys())
    }
    norm_comb_pair_freq_map = {
        key: scaled_comb_pair_freq_values[i][0]
        for i, key in enumerate(comb_pair_freq.keys())
    }
    ...
    for index, row in qdf.iterrows():
        ...
        for i in range(4):
            ...
            score = (
                w_pos * pos_score + w_pair * pair_score + w_comb_pair * comb_pair_score
            ...
            if i == gt_idx:
                ...
                max_distractor_comb_pair_score = max(
                    max_distractor_comb_pair_score, comb_pair_score
                )
        ...
        bias_info["relative_bias_pos_score"] = (
            bias_info["gt_pos_score"] - max_distractor_pos_score
        )
        bias_info["relative_bias_pair_score"] = (
            bias_info["gt_pair_score"] - max_distractor_pair_score
        )
        bias_info["relative_bias_comb_pair_score"] = (
            bias_info["gt_comb_pair_score"] - max_distractor_comb_pair_score
        )
...
def filter_app_order_relative_v2(
    ...
    Returns:
        List of question IDs selected for elimination.
    """
    df_with_scores = get_app_order_relative_score_v2(
        df, w_pos=w_pos, w_pair=w_pair, w_comb_pair=w_comb_pair
    )
...
def get_route_planning_score_v2(
    ...
    qdf[["beginning_object", "facing_object", "target_object"]] = qdf[
        ...
        r"You are a robot beginning (?:at|by) the (.*?) (?:facing the|facing to|facing towards the|facing|with your back to the) (.*?)\. You want to navigate to the (.*?)\."
    )
    ...
    # Extract Num Steps
    qdf["num_gt_steps"] = qdf["gt_route_str"].apply(
    ...
    # Object Frequencies
    all_objects = (
        pd.concat([qdf["beginning_object"], qdf["facing_object"], qdf["target_object"]])
        .dropna()
        .tolist()
    )
    ...
    norm_obj_freq_map = {
        k: v / len(all_objects) for k, v in obj_freq.items()
    ...
    qdf["steps_dist_norm"] = abs(qdf["num_gt_steps"] - mean_steps) / (
        std_steps + epsilon
    )
...
def filter_route_planning_v2(
    ...
    Returns:
        List of question IDs selected for elimination.
    """
    df_with_scores = get_route_planning_score_v2(
        df, w_obj_freq=w_obj_freq, w_route_freq=w_route_freq, w_steps_dist=w_steps_dist
    )
    ...
    for gt_answer_letter, count in gt_counts.items():
        ...
        if num_to_remove > 0:
            ...
            gt_samples = df_with_scores[
                df_with_scores["gt_answer_letter"] == gt_answer_letter
            ]
    ...
    if verbose:
        print(
            f"Total route_planning samples identified for full balancing: {total_candidates_for_balance}"
        )
        print(
            f"Actual number selected (prioritizing high secondary score): {len(final_remove_ids)}"
        )
    ...
    if len(final_remove_ids) > budget:
        ...
        while len(remove_ids) < budget:
            ...
            gt_answer_letter = max(
                gt_to_ids_to_remove, key=lambda k: len(gt_to_ids_to_remove[k])
            )
...
def debias_vsibench(save_path: str = "data/removed_ids.txt"):
    ...
    ids_obj_size = filter_object_size_estimation(
        df, budget=budgets["object_size_estimation"]
    )
    ids_abs_distance = filter_object_abs_distance(
        df, budget=budgets["object_abs_distance"]
    )
    ids_rel_distance = filter_object_rel_distance(
        df, budget=budgets["object_rel_distance"]
    )
    ids_obj_app_order = filter_obj_appearance_order(
        df, budget=budgets["obj_appearance_order"]
    )
    ids_obj_count = filter_object_counting(df, budget=budgets["object_counting"])
    ids_rel_dir = filter_object_rel_direction(
        df, budget=budgets["object_rel_direction"]
    )
    ids_room_size = filter_room_size_estimation(
        df, budget=budgets["room_size_estimation"]
    )
    ...
    for k, v in ids_map.items():
        ...
        print(
            f"{n_removed} / {n_orig} ({n_removed / n_orig * 100:.2f}%) \t of {k} removed{' ' * (25 - len(k))}\t => {n_remaining} / {n_orig} ({n_remaining / n_orig * 100:.2f}%) remaining"
        )

    print(sep)
    print(
        f"Number of ids removed: {len(all_ids)} / {len(df)} ({len(all_ids) / len(df) * 100:.2f}%)"
    )
    print(
        f"Number of ids remaining: {len(df) - len(all_ids)} / {len(df)} ({(len(df) - len(all_ids)) / len(df) * 100:.2f}%)"
    )
    print(sep)
